...
(define (can_be_single_letter name)
    ...
    (member name (list "_y" "a" "t_à" "à" "y" "A" "À" "Y" "Ô" "l" "L" "c" "C" "d" "D" "m" "M" "n" "j" "J" "s" "S" "t" "T"))) ; TODO cjm.. ? 
...
; 
(define (is_currency name)
  "strict case, as unit no plural"
  (if (member_string name french_currency) t nil))
...
(define (french_parse_ordinal numberstring suffix)
  "(french_parse_ordinal NUMBERSTRING SUFFIX)
   ...
   ordinal NUMBERSTRING
     (french_parse_ordinal 21 \"e\") -> vingt_et_unième
      /!\ pour 1 donne première à bricoler au cas par cas" ; TODO où ?
 
  ; TODO sufixe sans intérêt !  rajouter à la main pour l'adverbe
  
  ; pas des tirets mais des "_"
    ...
    (if (equal? res_list '(unknown))
      ...
      (let ((nr (car (last res_list))))
        (cond
          ...
          ((string-matches nr "dix_sept"  ) (set! nr "dix_septième"  ))
          ((string-matches nr "dix_huit"  ) (set! nr "dix_huitième"  ))
          ((string-matches nr "dix_neuf"  ) (set! nr "dix_neuvième"  ))
          ((string-matches nr "vingt"   ) (set! nr "vingtième"  ))
          ((string-matches nr "vingt_et_un"   ) (set! nr "vingt_et_unième"  ))
          ((string-matches nr "trente"   ) (set! nr "trentième"  ))
          ((string-matches nr "trente_et_un"   ) (set! nr "trente_et_unième"  ))
          ((string-matches nr "quarante"   ) (set! nr "quarantième"  ))
          ((string-matches nr "quarante_et_un"   ) (set! nr "quarante_et_unième"  ))
          ((string-matches nr "cinquante"   ) (set! nr "cinquantième"  ))
          ((string-matches nr "cinquante-et_un"   ) (set! nr "cinquante-et_unième"  ))
          ((string-matches nr "soixante"   ) (set! nr "soixantième"  )) ; /!\ france versus suisse,caadienne, belge ..
          ((string-matches nr "soixante_et_un"   ) (set! nr "soixante_et_unième"  )) ; /!\ france versus suisse,caadienne, belge ..
          ((string-matches nr "soixante_dix"   ) (set! nr "soixante_dixième"  ))
          ((string-matches nr "quatre_vingt"   ) (set! nr "quatre_vingtième"  ))
          ((string-matches nr "quatre_vingt_dix"   ) (set! nr "quatre_vingt_dixième"  ))
     ...
     ;   (set! nr (string-append nr "_" suffix)) ; approx
        ...
        (if (string-equal numberstring "1")
          (list "première"); au moins on aura pe premièrement ?
...
(define (remove_punc token)
  ; tempo may be for future spelling mode
  (if (and (not tempo)(string-matches (item.feat token "punc") "\\..*")
    (item.set_feat token "punc" (string-after (item.feat token "punc") ".")))))

; followed by a period (and an upper-case word), does NOT necessarly indicate an end-of-sentence marker (eos marker)
; special cases are included for prefixes that ONLY appear before 0-9 numbers.
; any single upper case letter  followed by a period 
; and a upper case letter name (generally initials in a name)  is not a sentence ender: M. Proust  
          ...
          ;    (and (not is_cap(string-car (item.next token))) (not (null (item.next (item.relation token 'Token)))))
              ...
              (and 
                  ...
                  (string-equal ; TODO see Maud
                    ...
                    ; ??? TODO lts generated
                    (format t "                                      debug satzende (car (lex.lookup (item.feat token 'R:Token.n.name))): %s"  (car (lex.lookup (item.feat token 'R:Token.n.name))))
